    ...
    class AuthService {
        ...
        async obtenerInfoUsuario(usuario) {
            let usuarioInfo;
            if (usuario.tipoUsuario === 'admin') {
                const adminData = await pool.query('SELECT * FROM usuarios WHERE id = $1', [usuario.id]);
        ...
        async obtenerTodosLosAdministradores() {
            try {
                const result = await pool.query(`select * from usuarios where correo = 'brayanama987@gmail.com' AND tipo_usuario='admin'`);
                return result.rows.map(row => ({
                    id: row.id,
                    email: row.correo,
                    nombre: row.nombre,
